    ...
    else if (waitingPacket)
    {
        ...
        if (bytes == DATA_SIZE_CRC)
        {
            ...
            if (packetReceivedID == waitingPacketSlaveID && packetReceived[1] == MY_LM_ID)
            {
                ...
                else if (calculatedCRC != packetCRC)
                {
                    Serial2.println("CRC incorrect");
                }
...
/**
 * @brief Stores packets from the slaves
 * 
 * @param ID 
 */
void LMStorePacket(int slaveID)
...
{
    Serial2.print("Telling the Slave ");
    ...
    if (slaveID < NUMBER_OF_SLAVES - 48) 
    {
        ...
        if (nextSlave == 1)
            ACCEPTING_COMMUNICATION_SV_X[1] = LOCAL_MASTER_ACCEPTING_COMMUNICATION_FROM_ALL_SLAVES;
        else
    ...
    sendPacket(ACCEPTING_COMMUNICATION_SV_X, sizeof(ACCEPTING_COMMUNICATION_SV_X) + 1); // AX-> local master accepting communication from Slave_X
}
